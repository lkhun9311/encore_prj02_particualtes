...
import com.spacetime.config.jwt.JwtAuthorizationFilter;
...
//		http.addFilterBefore(new MyFilter3(), BasicAuthenticationFilter.class);
		...
		.addFilter(new JwtAuthorizationFilter(authenticationManager(), userRepository)) // AuthenticationManager
				...
				.antMatchers("/api/v1/admin/**").access("hasRole('ROLE_ADMIN')").anyRequest().permitAll().and().logout()
				.logoutSuccessUrl("/") // 로그아웃 성공시 리다이렉트 주소
				.deleteCookies("JESSIONID") // 로그아웃 후 쿠키 삭제
				.deleteCookies("Authorization") // 로그아웃 후 쿠키 삭제
